...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Generate standard P2P UPI URI for general app links
    function buildAppUpiUri(scheme, amount) {
        const u = config.upiId.trim();
        const n = encodeURIComponent(config.payeeName.trim());
        const tn = encodeURIComponent(config.reasonNote.trim());
        const am = amount.toFixed(2);
        
        const params = `pa=${u}&pn=${n}&am=${am}&tn=${tn}&cu=INR`;
    ...
    // Generate standard unencoded P2P UPI URI for QR codes (highest compatibility)
    ...
    // Helper to generate exact Direct-Launch UPI Deep Link matching standard P2P specifications
    function getPreciseMobileUpiDeepLink(scheme, amount) {
        ...
        // Base standard P2P UPI parameters
        const params = `pa=${u}&pn=${n}&am=${am}&tn=${tn}&cu=INR`;
