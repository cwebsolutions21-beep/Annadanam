...
document.addEventListener('DOMContentLoaded', () => {
    ...
    formDonate.addEventListener('submit', (e) => {
        ...
        // Construct UPI URIs
        const upiLinkPhonePe = buildAppUpiUri('phonepe', donorAmount);
        const upiLinkGPay = buildAppUpiUri('gpay', donorAmount);
        const upiLinkPaytm = buildAppUpiUri('paytm', donorAmount);
        const upiLinkGeneric = buildAppUpiUri('upi', donorAmount);
        
        // 1. Desktop Setup (Dynamic QR Code Generation)
        // Decoupled QR URI with literal spaces for maximum reader compatibility
        const qrUpiLink = buildQrUpiUri(donorAmount);
        ...
        // then switch directly to the verification simulator in the background.
        if (isMobile) {
    ...
    // Generate UPI URI according to NPCI / App specs
    function buildAppUpiUri(scheme, amount) {
        ...
        if (scheme === 'phonepe') {
            return `phonepe://pay?pa=${u}&pn=${n}&am=${am}&tn=${tn}&cu=INR`;
        } else if (scheme === 'gpay') {
            return `gpay://upi/pay?pa=${u}&pn=${n}&am=${am}&tn=${tn}&cu=INR`;
        } else if (scheme === 'paytm') {
            return `paytm://pay?pa=${u}&pn=${n}&am=${am}&tn=${tn}&cu=INR`;
        } else {
            return `upi://pay?pa=${u}&pn=${n}&am=${am}&tn=${tn}&cu=INR`;
        }
    }

    // Generate raw unencoded UPI URI specifically for standard QR codes (preserves spaces for scanners)
    ...
    // Fallback back button on verification page
    if (btnVerifyBack) {
